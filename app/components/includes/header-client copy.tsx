...
const HeaderClient: React.FC = () => {
    ...
    return (
        <div className="bg-[#045b27] sticky top-0 z-20 shadow-md shadow-black/35">
                    ...
                    <div className="relative px-6 py-4 lg:pl-8 lg:pr-8">
                        <nav className="flex items-center justify-between sm:h-10 lg:justify-between" aria-label="Global">
                            ...
                            <div className="hidden lg:ml-12 lg:space-x-10 lg:flex">
                                {navigation.map((item) => (
                                    <Link key={item.name} href={item.href} className=" px-4 py-2 text-sm font-bold leading-6 text-white flex justify-center items-center gap-x-2">
                                        {item.icon}
                                        {item.name}
                                    </Link>
                                ))}
                            ...
                            {isAuthenticated ? (
                                <Popover className="hidden md:block">
                                    <PopoverButton className={` ${isAuthenticated ? ' bg-[#f7872e] rounded-2xl' : ''} px-4 py-2 text-sm/6 font-bold text-white focus:outline-none`}>
                                        Mon compte
                                    </PopoverButton>
                                    ...
                                    <PopoverPanel transition anchor="bottom" className="divide-y z-50 divide-white/5 rounded-xl bg-black text-sm/6 transition duration-200 ease-in-out">
                                        <div className="p-3">
                                            <a className="block rounded-lg py-2 px-3 transition hover:bg-[#f7872e]" href={href + '/settings'}>
                                                <p className="font-bold text-white">Gérer mon compte</p>
                                            </a>
                                        </div>

                                        <div className="p-3">
                                            <button onClick={handleSignOut} className="block rounded-lg py-2 px-3 transition hover:bg-[#f7872e]">
                                                <p className="font-bold text-white">Déconnexion</p>
                                            </button>
                                        </div>
                                    </PopoverPanel>
                                </Popover>
                        ...
                        <Dialog open={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)}>
                            <DialogPanel className="fixed inset-0 z-40 overflow-y-auto bg-white px-6 py-6 lg:hidden">
                                ...
                                <div className="mt-3 space-y-2">
                                    ...
                                    {navigation.map((item) => (
                                        <Link key={item.name} href={item.href} onClick={() => setMobileMenuOpen(false)} className="-mx-3 rounded-lg py-2 px-3 text-base font-bold leading-7 text-black hover:bg-gray-400/10 flex items-center gap-x-2">
                                            {item.icon}
                                            {item.name}
                                        </Link>
                                    ))}
                                    ...
                                    {isAuthenticated ? (
                                        ...
                                        <>
                                            <Link href={href + '/settings'} onClick={() => setMobileMenuOpen(false)} className="mx-3 rounded-lg py-2 px-3 text-base font-bold leading-7 text-black hover:bg-gray-400/10 flex items-center gap-x-2">
                                                Gérer mon compte
                                            </Link>

                                            <button onClick={handleSignOut} className="mx-3 rounded-lg py-2 px-3 text-base font-bold leading-7 text-black hover:bg-gray-400/10 flex items-center gap-x-2">
                                                <p className="font-bold text-black">Déconnexion</p>
                                            </button>
                                        </>
                                    ) : (

                                        <Link href="/login" className="mx-3 rounded-lg py-2 px-3 text-bold font-bold leading-7 text-black hover:bg-gray-400/10 flex items-center gap-x-2">
