...
const Map: React.FC<MapProps> = ({ coordinates, title }) => {
  ...
  // Fonction pour calculer le zoom et le centre
  ...
  // Hook pour calculer le zoom, le centre, et les itinéraires dès que les coordonnées changent
  useEffect(() => {
    if (coordinates.length > 0) {
      calculateZoomAndCenter(coordinates);
      calculateRoute();
          ...
          >
            ...
            {coordinates.map((coord, index) => (
              <Marker
                key={index}
                position={{ lat: coord.lat, lng: coord.lng }}
                icon={index === 0 ? { url: 'https://i.ibb.co/cyvcpfF/uberx.png', scaledSize: { width: 32, height: 32 } } : undefined}
                title={`Point ${index + 1}`}
              />
            ))}
          </GoogleMap>
